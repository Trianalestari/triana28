...
class Auth extends CI_Controller 
{
	...
	private function _login()
	{
		...
		if ($user) {
			...
			if ($user['is_active'] == 1) {
				...
				$this->session->set_flashdata(
					'message',
					'<div class="alert alert-danger" role="aleart">
					This email has not been actvated!
				</div>'
				);
				redirect('auth');	
			}
	...
	public function registration()
	{
		...
		[
		  ...
		  'main_length' => 'password to shoort'
		]
	);
		...
		$this->session->set_flashdata('massage','<div class="alert alert-seccess" role="alert">Congratulations, your account has been created. Please login!
	    </div>');
		redirect('auth'); 
	...
	private function _sendEmail($token, $type)
    {
        ...
        $config['smtp_user'] = 'abimanyu1234poa@gmail.com'; // isi dgn email
        $config['smtp_pass'] = 'nzecgvdvmuszcpys'; // password harus sesuai
        $config['smtp_port'] = '465';
        $config['malltype'] = 'html';
        $config['charset'] = 'utf-8';
        $config['newline'] = "\r\n";
        $this->email->initialize($config);
        $this->email->from('abimanyu1234poa@gmail.com', 'wahyu');
	...
	public function resetpassword()
	{
		...
		if($user) {
			...
			$this->session->set_flashdata('message', 
			'<div class="alert alert role="alert">Reset password failed!Wrong email!</div');
			redirect('auth/forgotpassword');
		}
	}
	public function changePassword()
	{
		...
		$this->form_validation->set_rules('password2', 'Repeat Password', 'trim|required|min_lenght[3]|matches[password]');
		if ($this->form_validation->run() == false) {
			...
			role="alert">Password has been change!Please login!</div>');
			redirect('auth');
		}
	}
